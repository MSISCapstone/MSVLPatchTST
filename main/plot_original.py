...
from PatchTST_supervised.utils.metrics import MAPE, HuberLoss
...
def save_stats_and_plot(preds, trues, data_columns, out_dir, seq_len, pred_len):
    ...
    fig.suptitle(f'Original PatchTST Weather Predictions - {total_timesteps} Timesteps\n({num_samples} samples x {actual_pred_len} steps, seq_len={seq_len})', fontsize=14)
    ...
    with open(Path(out_dir) / 'summary.txt', 'w') as fh:
        ...
        if target_indices:
            ...
            # Overall MAPE with protection against division by zero
            ...
            overall_huber = overall_mape = float('nan')
