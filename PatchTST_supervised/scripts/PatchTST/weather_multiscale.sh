...
# Multi-Scale PatchTST for Weather Forecasting (Channel-Independent)
# Uses variable-length patches to capture temporal patterns at multiple scales:
...
# Enable multi-scale patching (channel-independent)
multi_scale=1
channel_independent=1        # Channel-independent for memory efficiency
...
echo "Running Multi-Scale PatchTST (Channel-Independent)"
echo "==================================================="

for pred_len in 336 720
do
    log_file="/content/logs/LongForecasting/${model_name}_MultiScale_${model_id_name}_${seq_len}_${pred_len}.log"
    
    echo "========================================" | tee "$log_file"
    echo "Multi-Scale PatchTST (Channel-Independent)" | tee -a "$log_file"
